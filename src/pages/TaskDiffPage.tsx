...
export function TaskDiffPage() {
  ...
  const markdownFields = new Set<keyof TaskSnapshot>(["outcome"]);
  ...
  function renderFieldValue(key: keyof TaskSnapshot, raw: string | null): React.ReactNode {
          ...
          {fields.map(({ key, label }) => {
            ...
            return isChanged ? (
              <FieldRow
                key={key}
                label={label}
                prev={renderFieldValue(key, prevRaw)}
                curr={renderFieldValue(key, currRaw)}
              />
            ) : (
